...
/* ----------------------- Defines ------------------------------------------*/
/* ----------------------- Variables ----------------------------------------*/
static osSemaphoreId xMasterRunRes;
...
//	printf("\r\nxMBMasterPortEventPost ---> %d",eEvent);
...
//            &recvedEvent);
	...
	recvedEvent = xEventGroupWaitBits(xMasterOsEvent, EV_MASTER_READY | EV_MASTER_FRAME_RECEIVED 
		| EV_MASTER_EXECUTE | EV_MASTER_FRAME_SENT | EV_MASTER_ERROR_PROCESS,
...
//    rt_sem_init(&xMasterRunRes, "master res", 0x01 , RT_IPC_FLAG_PRIO);
	printf("\r\nvMBMasterOsResInit");
	osSemaphoreDef(master_res);
	xMasterRunRes = osSemaphoreCreate(osSemaphore(master_res), 1);
}
...
//    return rt_sem_take(&xMasterRunRes, lTimeOut) ? FALSE : TRUE ;
	return (osSemaphoreWait(xMasterRunRes, lTimeOut)) ? FALSE : TRUE;
}
...
//            &recvedEvent);
	EventBits_t recvedEvent;
	recvedEvent = xEventGroupWaitBits(xMasterOsEvent, EV_MASTER_PROCESS_SUCESS | EV_MASTER_ERROR_RESPOND_TIMEOUT 
		| EV_MASTER_ERROR_RECEIVE_DATA | EV_MASTER_ERROR_EXECUTE_FUNCTION,
            pdTRUE, pdFALSE, portMAX_DELAY);
	printf("\r\neMBMasterWaitRequestFinish: %d",recvedEvent);
    switch (recvedEvent)
    {
